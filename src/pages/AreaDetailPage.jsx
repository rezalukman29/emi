  ...
  if (!data) {
    return (
      <div style={{ padding:40, textAlign:'center' }}>
        <p style={{ color:'var(--text-muted)', fontSize:14 }}>{t("wording.areaNotFound")}</p>
        <button className="btn-new" style={{ marginTop:12 }} onClick={() => navigate('/area')}>{t("wording.backToArea")}</button>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        <div className="card" style={{ padding:24 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:20 }}>
            ...
            <span style={{ fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.07em' }}>{t("wording.areaInfo")}</span>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:18 }}>
            <Field label={t("wording.name")} value={data?.data?.name} />
            <Field label={t("wording.subAreas")} value={String(data?.data?.list_sub_area?.length ?? 0)} />
            <Field label={t("wording.createdAt")} value={fmtDate(data?.data?.created_at)} />
        ...
        <div className="stats-bar" style={{ gridTemplateColumns:'1fr', alignContent:'start', gap:12, background:'transparent', border:'none', padding:0 }}>
          {[
            { label:t("wording.totalSubAreas"), value: data?.data?.list_sub_area?.length ?? "0", color:'var(--brand)',  bg:'var(--brand-bg)' },
          ].map(s => (
      ...
      {data?.data?.list_sub_area?.length > 0 && (
        <div className="card">
          <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--border)', fontWeight:700, fontSize:14 }}>{t("wording.subAreas")}</div>
      ...
      <Modal open={deleteOpen} title={t("wording.deleteArea")} onClose={() => setDeleteOpen(false)}
        footer={<>
          <button className="btn-cancel-modal" onClick={() => setDeleteOpen(false)}>{t("wording.cancel")}</button>
          <button className="btn-del-ok" onClick={() => { setDeleteOpen(false); navigate('/area'); }}>{t("wording.delete")}</button>
        </>}
      >
        <p className="confirm-msg">{t("wording.areYouSureYouWantToDelete")} <strong>{data?.data?.name}</strong>{t("wording.thisActionCannotBeUndone")}</p>
      </Modal>
    </>
  );
}
